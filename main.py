...
def load_streaming_history() -> List[dict]:
    ...
    for file in files:
        with open(file, "r") as f:
            ...
            streams += [s for s in new_streams]
            f.close()
...
def get_ignored_artists(streams: List[dict]) -> List[str]:
    ...
    artists = sorted({ s["artistName"] for s in streams })

    questions = [
        inquirer.Checkbox(
            "ignored_artists",
            message="Which artists would you like to ignore?",
            choices=artists,
...
def clean_exported_data(ignored_artist: List[str]):
    ...
    for file in files:
        with open(file, "r", encoding="UTF-8") as f:
            streams = json.load(f)
            cleaned_streams = [s for s in streams if s["artistName"] not in ignored_artist]
            ...
            print(f"Cleaned {f.name} -> {cleaned_file_name}")
